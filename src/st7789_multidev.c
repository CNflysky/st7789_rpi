...
void st7789_multidev_set_display_area(st7789_t *st7789, uint16_t x0,
...
void st7789_multidev_send_buf(st7789_t *st7789) {
  st7789_multidev_set_display_area(st7789, 0, 0, st7789->width - 1,
                                   st7789->height - 1);
...
void st7789_multidev_clear_screen(st7789_t *st7789) {
  st7789_multidev_fill_screen(st7789, BLACK);
}

void st7789_multidev_draw_line(st7789_t *st7789, uint16_t x1, uint16_t y1,
  ...
  for (uint16_t t = 0; t <= distance + 1; t++) {
    st7789_multidev_draw_pixel(st7789, uRow, uCol, color);
    xerr += delta_x;
    yerr += delta_y;
    if (xerr > distance) {
      xerr -= distance;
...
static void st7789_multidev_precircle(st7789_t *st7789, uint16_t xc,
                                      uint16_t yc, uint16_t x, uint16_t y,
                                      uint16_t color) {
  st7789_multidev_draw_pixel(st7789, xc + x, yc + y, color);
  st7789_multidev_draw_pixel(st7789, xc - x, yc + y, color);
  st7789_multidev_draw_pixel(st7789, xc + x, yc - y, color);
  st7789_multidev_draw_pixel(st7789, xc - x, yc - y, color);
  st7789_multidev_draw_pixel(st7789, xc + y, yc + x, color);
  st7789_multidev_draw_pixel(st7789, xc - y, yc + x, color);
  st7789_multidev_draw_pixel(st7789, xc + y, yc - x, color);
  st7789_multidev_draw_pixel(st7789, xc - y, yc - x, color);
}
...
void st7789_multidev_draw_circle(st7789_t *st7789, uint16_t xc, uint16_t yc,
  ...
  if (filled) {
    while (x <= y) {
      for (yi = x; yi <= y; yi++)
        st7789_multidev_precircle(st7789, xc, yc, x, yi, color);
      if (fontwidth < 0)
    ...
    while (x <= y) {
      st7789_multidev_precircle(st7789, xc, yc, x, y, color);
      if (fontwidth < 0)
        fontwidth = fontwidth + 4 * x + 6;
      else {
        fontwidth = fontwidth + 4 * (x - y) + 10;
  ...
  if (filled) {
    for (uint16_t i = y1; i < y2; i++)
      for (uint16_t j = x1; j < x2; j++)
        st7789_multidev_draw_pixel(st7789, j, i, color);
  } else {
    st7789_multidev_draw_line(st7789, x1, y1, x2, y1, color);
    st7789_multidev_draw_line(st7789, x1, y1, x1, y2, color);
    st7789_multidev_draw_line(st7789, x1, y2, x2, y2, color);
    st7789_multidev_draw_line(st7789, x2, y1, x2, y2, color);
  }
}

void st7789_multidev_draw_chinese_string(st7789_t *st7789, gt30_t *gt30,
                                         uint16_t x, uint16_t y, fonts_t font,
  ...
  for (uint8_t i = 0; i < strlen(str) / 3; i++) {
    ...
    st7789_multidev_draw_chinese_char(st7789, gt30, x0, y, font, buf, fcolor);
    offset += 3;
    x0 += distance;
  }
}

static void st7789_multidev_draw_font_data(st7789_t *st7789, uint16_t x,
                                           uint16_t y, uint16_t fontwidth,
                                           uint16_t fontheight,
...
void st7789_multidev_draw_chinese_char(st7789_t *st7789, gt30_t *gt30,
                                       uint16_t x, uint16_t y, fonts_t type,
                                       uint8_t *ch, uint16_t fcolor) {
  ...
  usleep(1000);  // 1ms
  st7789_multidev_draw_font_data(st7789, x, y, fontwidth, fontwidth, fontsize,
                                 data, fcolor, 0);
}
...
void st7789_multidev_draw_ascii_char(st7789_t *st7789, gt30_t *gt30, uint16_t x,
  ...
  st7789_multidev_draw_font_data(st7789, x, y, fontwidth, fontheight, fontsize,
                                 buf, fcolor, spec);
}

void st7789_multidev_draw_ascii_string(st7789_t *st7789, gt30_t *gt30,
                                       uint16_t x, uint16_t y, fonts_t font,
                                       uint8_t *str, uint16_t fcolor) {
  uint8_t distance = st7789_gt30_get_font_param(font, WIDTH), x0 = x;
  for (uint8_t i = 0; i < strlen(str); i++) {
    st7789_multidev_draw_ascii_char(st7789, gt30, x0, y, font, str[i], fcolor);
    x0 += distance;
  }
}

void st7789_multidev_draw_string(st7789_t *st7789, gt30_t *gt30, uint16_t x,
                                 uint16_t y, fonts_t ascfont, fonts_t cnfont,
  ...
  for (uint16_t i = 0; i < strlen(str); i++) {
    if (str[i] >= 0x20 && str[i] <= 0x7E) {
      st7789_multidev_draw_ascii_char(st7789, gt30, x0, y, ascfont, str[i],
                                      color);
      x0 += asc_distance;
    } else {
      memcpy(cn_buf, &str[i], 3);  // chinese
      i += 2;
      st7789_multidev_draw_chinese_char(st7789, gt30, x0, y, cnfont, cn_buf,
                                        color);
      x0 += cn_distance;
    }
  }
...
void st7789_multidev_printf(st7789_t *st7789, gt30_t *gt30, uint16_t x,
                            uint16_t y, fonts_t ascfont, fonts_t gbfont,
  ...
  va_end(arg);
  st7789_multidev_draw_string(st7789, gt30, x, y, ascfont, gbfont, buf, color);
}
